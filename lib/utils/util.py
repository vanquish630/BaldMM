...
def write_seperated_obj(obj_name,
              vertices,
              faces,
              colors=None,
              texture=None,
    ...
    mtl_name = obj_name.replace('.obj', '.mtl')
    texture_name = obj_name.replace('.obj', '.png')
    iris_texture_name = f"{obj_name.split('.')[0]}_iris.png"
    material_name = 'FaceTexture'
    material_name_head = 'FaceTexture'
    material_name_eye_right = 'EyeRightTexture'
    material_name_eye_left = 'EyeLeftTexture'
    material_name_iris = "iris"

    # texture = texture.transpose(1, 2, 0)
    # print(texture.max())
    # texture = texture.clip(0, 1)
    # texture = (texture * 255).astype('uint8')

    eye_face_indices = np.load(os.path.join('./data/seperated_obj', 'eye_face_indices.npy')).tolist()
    eye_left_indices = np.load(os.path.join('./data/seperated_obj', 'eye_left.npy')).tolist()
    eye_right_indices = np.load(os.path.join('./data/seperated_obj', 'eye_right.npy')).tolist()
    iris_right_indices = np.load(os.path.join('./data/seperated_obj', 'irisRight4.npy')).tolist()
    iris_left_indices = np.load(os.path.join('./data/seperated_obj', 'irisLeft4.npy')).tolist()

    eyeFaces = np.array(eye_face_indices)
    eyeLeftIndices = np.array(eye_left_indices)
    eyeRightIndices = np.array(eye_right_indices)
    irisLeftIndices = np.array(iris_left_indices)
    irisRightIndices = np.array(iris_right_indices)

    irisRightFile = os.path.join('./data/seperated_obj', "irisRight.txt")
    irisLeftFile = os.path.join('./data/seperated_obj', "irisLeft.txt")

    # vertices = vertices.cpu().numpy()
    # faces = faces.cpu().numpy()
    faces = faces.copy()
    # print("tex" , texture.shape)
    # mesh lab start with 1, python/c++ start from 0
    faces += 1
    if inverse_face_order:
        faces = faces[:, [2, 1, 0]]
    ...
    # write obj
    with open(obj_name, 'w') as f, open(irisRightFile, 'r', encoding='utf-8') as ir, open(irisLeftFile, 'r',
                                                                                          encoding='utf-8') as il:
        # first line: write mtlib(material library)
        # f.write('# %s\n' % os.path.basename(obj_name))
        # f.write('#\n')
        # f.write('\n')
        ...
        # write uv coords
        if texture is None:
            for i in range(7800, 8888):
                f.write('f {} {} {}\n'.format(faces[i, 2], faces[i, 1], faces[i, 0]))
        else:
            for i in range(3962, 4540):
                f.write('vt {} {}\n'.format(uvcoords[i, 0], uvcoords[i, 1]))

            f.write('usemtl %s\n' % material_name_eye_right)
            # write f: ver ind/ uv ind
            for i in eyeRightIndices:
                f.write('f {}/{} {}/{} {}/{}\n'.format(
                    #  faces[i, 2], uvfaces[i, 2],
                    #  faces[i, 1], uvfaces[i, 1],
                    #  faces[i, 0], uvfaces[i, 0]
        ...
        # write uv coords
        if texture is None:
            for i in range(8888, 9976):
                f.write('f {} {} {}\n'.format(faces[i, 2], faces[i, 1], faces[i, 0]))
        else:
            for i in range(4540, 5118):
                f.write('vt {} {}\n'.format(uvcoords[i, 0], uvcoords[i, 1]))

            f.write('usemtl %s\n' % material_name_eye_left)
            # write f: ver ind/ uv ind
            for i in eyeLeftIndices:
                f.write('f {}/{} {}/{} {}/{}\n'.format(
                    #  faces[i, 2], uvfaces[i, 2],
                    #  faces[i, 1], uvfaces[i, 1],
                    #  faces[i, 0], uvfaces[i, 0]
            ...
            f.write('o iris_right\n')

            # write vertices
            for i in irisRightIndices:
                f.write('v {} {} {}\n'.format(vertices[i, 0], vertices[i, 1], vertices[i, 2]))

            irisRightCenter = np.mean(vertices[irisRightIndices], axis=0)
            f.write('v {} {} {}\n'.format(irisRightCenter[0], irisRightCenter[1], irisRightCenter[2]))
            ...
            # write vertices
            for i in irisLeftIndices:
                f.write('v {} {} {}\n'.format(vertices[i, 0], vertices[i, 1], vertices[i, 2]))

            irisLeftCenter = np.mean(vertices[irisLeftIndices], axis=0)
            f.write('v {} {} {}\n'.format(irisLeftCenter[0], irisLeftCenter[1], irisLeftCenter[2]))
            ...
            with open(mtl_name, 'w') as f:
                ...
                f.write('newmtl %s\n' % material_name_iris)
                f.write('Ka 1.000000 1.000000 1.000000\n')
                s = 'map_Kd {}\n'.format(os.path.basename(iris_texture_name))  # map to image
                f.write(s)
